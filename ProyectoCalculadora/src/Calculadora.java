...
public class Calculadora {
	...
	public void guardar(String calc,int guar){
		...
		for(int i=0;i<calc.length();i++){
			//EL CARACTER ES UN NUMERO Y NO ESTAMOS AL FINAL DEL STRING INGRESADO?..VUELVE A ENTRAR AL FOR CON i+1
			if(esNum(calc.charAt(i))&& i!=calc.length()-1){
				continue;
			//SI EL CARACTER ES UN NUMERO PERO ESTAMOS AL FINAL QUIERE DECIR QUE NO HAY MAS OPERADORES ENTONCES GUARDO EL NUMERO
			}else if(esNum(calc.charAt(i))&& i==calc.length()-1){
				numeros[guar]= calc;
				return;
				
			}
			//SI EL CARACTER ES OPERADOR GUARDO EL OPERADOR Y EL NUMERO Y CORTO EL STRING Y VUELVO A LLAMAR AL METODO
	...
	public String calcular(String calc,int resultado,int i){
		
		int a=0;
		int b =0;
		//EL ARRAY ESTA LLENO DE a's, si llegue a una quiere decir que no hay mas operadores
		if(operadores[i]=='a'){
		...
		//SI NO ES LA PRIMERA VEZ LE SUMO O RESTO EL SIGUIENTE NUMERO AL OBTENIDO ANTERIORMENTE
		}else if(i>0){
			switch(oper){
			case '+':
				resultado+=a;
